...
class ImageViewer(object):
    ...
    * 'ESC' : quit
    ...
    def __init__(self, update_ms, window_shape=(640, 480), caption="Figure 1", camera_num=1):
        ...
        self.image = [np.zeros(self._window_shape + (3, ), dtype=np.uint8) for i in range(camera_num)]
        # self.image = []
        self._color = (0, 0, 0)
    ...
    def rectangle(self, x, y, w, h, index,label=None):
        """Draw a rectangle.
        ...
        pt1 = int(x), int(y)
        pt2 = int(x + w), int(y + h)
        cv2.rectangle(self.image[index], pt1, pt2, self._color, self.thickness)
        if label is not None:
            ...
            pt2 = pt1[0] + 10 + text_size[0][0], pt1[1] + 10 + \
                text_size[0][1]
            cv2.rectangle(self.image[index], pt1, pt2, self._color, -1)
    ...
    def run(self, vis,tracker_dic,update_fun=None):
